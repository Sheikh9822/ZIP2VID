...
# 3 images per second = 0.333s duration
DURATION = 0.333 
FPS = 15 
TARGET_BITRATE = "400k"
AUDIO_BITRATE = "96k"
...
def main():
    ...
    # 4. HIGH SPEED ANIMATED ENCODING
    out_video = f"output/{FILENAME}.mkv"
    
    # Calculation: 15 FPS / 3 images per sec = 5 frames per image
    frames_per_img = 5
    
    # Fast zoompan for high-speed montage (zoom+0.01 is faster than 0.001)
    vf = (
        f"zoompan=z='min(zoom+0.008,1.3)':d={frames_per_img}:s={W}x{H}:fps={FPS},"
    ...
    cmd = (
        f'ffmpeg -y -framerate 3 -i workspace/processed/img_%04d.jpg {audio_input} '
        f'-vf "{vf}" '
    ...
    print(f"Encoding Montage (3 img/sec, {TARGET_BITRATE})...")
